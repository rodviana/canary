...
taskHuntingLimitedTasksExhaust = 2 * 60 * 60
taskHuntingRerollPricePerLevel = 40
...
-- partyShareRangeMultiplier: higher = party shared XP with larger level gaps (default 1.5)
partyShareRangeMultiplier = 15
...
-- NOTE: toggleInfiniteCapacity = true ignores carry weight for all players (same effect as group flag HasInfiniteCapacity).
...
-- Map
-- mapName: sem .otbm; ficheiro local = dataPackDirectory/world/{mapName}.otbm
-- toggleDownloadMap: se true e esse ficheiro NÃO existir, o servidor faz GET em mapDownloadUrl e grava nesse caminho.
-- mapDownloadUrl: URL direta do asset (ex. GitHub Release):
--   https://github.com/OWNER/REPO/releases/download/TAG_DO_RELEASE/nome_do_asset.otbm
-- Repositório privado: o Canary não envia token; use mapa local ou URL pública.
toggleDownloadMap = true
mapDownloadUrl = "https://github.com/opentibiabr/canary/releases/download/v3.4.1/otservbr.otbm"
mapName = "otservbr"
mapAuthor = "rodviana"
...
-- When true, logs detailed map load phases, timings, OTBM counters, duplicate tile/waypoint warnings, and external spawn file paths
toggleMapLoadVerbose = true
...
rateKillingInTheNameOfPoints = 5
...
rateExerciseTrainingSpeed = rateAttackSpeed * 5
